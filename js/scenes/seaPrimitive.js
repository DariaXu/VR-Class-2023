...
export const init = async model => {
    ...
    model.animate(() => {
        ...
        if (joyStickX!=0){
            if (joyStickX>0){
                speedX = Math.min(maxSpeed, speedX + accX);
                //speedX = Math.min(maxSpeed, speedX + (joyStickX - prevX) * 0.005);
            }
            else if (joyStickX<0){
                speedX = Math.max(-maxSpeed, speedX - accX);
            ...
            if (speedX>0){
                speedX = Math.max(0, speedX *.95 - eps);
            }
            else if (speedX<0){
                speedX = Math.min(0, speedX *.95 + eps);
            }
            else
                speedX = 0;
            accX = .003;
        ...
        if (joyStickY!=0){
            if (joyStickY>0){
                speedY = Math.min(maxSpeed, speedY + accY);
                //speedY = Math.min(maxSpeed, speedY + (joyStickY - prevY) * 0.005);
            }
            else if (joyStickY<0){
                speedY = Math.max(-maxSpeed, speedY - accY);
            ...
            if (speedY>0){
                speedY = Math.max(0, speedY *.95 - eps);
            }
            else if (speedY<0){
                speedY = Math.min(0, speedY *.95 + eps);
            }
            else{
                speedY = 0;
            }

...
export const init = async model => {
    ...
    model.animate(() => {
        ...
        if (joyStickX!=0){
            if (joyStickX>0){
                speedX = Math.min(maxSpeed, speedX + 0.004);
                //speedX = Math.min(maxSpeed, speedX + (joyStickX - prevX) * 0.005);
            }
            else if (joyStickX<0){
                speedX = Math.max(-maxSpeed, speedX - 0.004);
            ...
            if (speedX>0){
                speedX = Math.max(0, speedX - 0.003);
            }
            else if (speedX<0){
                speedX = Math.min(0, speedX + 0.003);
            }
            else
                speedX = 0;
        }
        if (joyStickY!=0){
            if (joyStickY>0){
                speedY = Math.min(maxSpeed, speedY + 0.004);
                //speedY = Math.min(maxSpeed, speedY + (joyStickY - prevY) * 0.005);
            }
            else if (joyStickY<0){
                speedY = Math.max(-maxSpeed, speedY - 0.004);
                // speedY = Math.max(-maxSpeed, speedY + (joyStickY - prevY) * 0.005);
            }
        }
        else {
            if (speedY>0){
                speedY = Math.max(0, speedY - 0.003);
            }
            else if (speedY<0){
                speedY = Math.min(0, speedY + 0.003);
            }
            else
                speedY = 0;
        }
